...
from ..schemas import CreateAuditorium, UpdateAuditorium, DeleteAuditorium
...
async def create_auditorium(auditorium_model: CreateAuditorium) -> Auditorium:
    ...
    if existing:
        raise HTTPException(
            status_code=409,
            detail=f"Auditorium with identi fier '{auditorium_model.identifier}' already exists."
        )
...
async def get_auditorium_by_uuid_equip(auditorium_uuid: UUID4) -> Optional[Auditorium]: # Возвращаем Optional[Model]
    auditorium = await Auditorium.get_or_none(uuid=auditorium_uuid).prefetch_related('equipment')
    # Проверка на 404 теперь в роутере или при использовании результата
    return auditorium
