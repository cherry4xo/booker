...
def create_keyboard(page_num, aud_names):
    """Создает инлайн-клавиатуру для текущей страницы."""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    total_pages = (len(aud_names) + 9 - 1) // 9

    # Вычисляем индексы элементов для отображения на текущей странице
    ...
    for i in range(start_index, end_index):
        ...
        row.append(button)
        if len(row) == 3:  # Добавляем по 3 кнопки в ряд
            keyboard.inline_keyboard.append(row)
            row = []
    if row: # Если остались кнопки после цикла, добавляем их
        keyboard.inline_keyboard.append(row)
    ...
    keyboard.inline_keyboard.append(nav_row)  # Добавляем кнопки навигации
